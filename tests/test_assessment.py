...
async def sample_assessment(db_session: AsyncSession):
    ...
    await db_session.exec(
        text(
            "INSERT INTO assessments (id, title, subject) VALUES (:id, :title, :subject)"
        ),
        params={
            "id": assessment_id,
...
async def valid_token(db_session: AsyncSession, sample_student, sample_assessment):
    ...
    await db_session.exec(
        text(
            "INSERT INTO tokens (token, student_id, assessment_id, is_used, expires_at) VALUES (:token, :student_id, :assessment_id, FALSE, :expires_at)"
        ),
        params={
            "token": token_value,
...
async def used_token(db_session: AsyncSession, sample_student, sample_assessment):
    ...
    await db_session.exec(
        text(
            "INSERT INTO tokens (token, student_id, assessment_id, is_used) VALUES (:token, :student_id, :assessment_id, TRUE)"
        ),
        params={
            "token": token_value,
            "student_id": sample_student,
...
class TestGetAssessment:
    async def test_retorna_contexto_com_token_valido(
        self, valid_token, client: AsyncClient
    ):
    ...
    async def test_retorna_410_para_token_ja_usado(
        self, used_token, client: AsyncClient
    ):
        response = await client.get(f"/api/assessment/{used_token}")
        assert response.status_code == 410
        assert "utilizado" in response.json()["detail"].lower()

    async def test_retorna_410_para_token_expirado(
        self, expired_token, client: AsyncClient
    ):
...
class TestSubmitAssessment:
    async def test_submissao_salva_respostas_e_invalida_token(
        self, valid_token, db_session: AsyncSession, client: AsyncClient
    ):
        payload = [
            {
        ...
        response = await client.post(
            f"/api/assessment/{valid_token}/submit", json=payload
        )
    ...
    async def test_submissao_com_respostas_qualitativas_sem_score(
        self, valid_token, client: AsyncClient
    ):
        payload = [
    ...
    async def test_segundo_submit_com_mesmo_token_e_rejeitado(
        self, valid_token, client: AsyncClient
    ):
    ...
    async def test_submit_com_token_inexistente_retorna_404(self, client: AsyncClient):
        ...
        response = await client.post(
            "/api/assessment/token_falso_xyz/submit", json=payload
        )
        assert response.status_code == 404

    async def test_submit_sem_respostas_retorna_422(
        self, valid_token, client: AsyncClient
    ):
